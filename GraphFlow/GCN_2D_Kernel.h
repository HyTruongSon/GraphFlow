// Framework: GraphFlow
// Class: Generalized Steerable Convolutional Networks (Second-order representation) for Kernel Learning
// Author: Machine Learning Group of UChicago
// Institution: Department of Computer Science, The University of Chicago
// Copyright 2017 (c) UChicago. All rights reserved.

#ifndef __GCN_2D_KERNEL_H_INCLUDED__
#define __GCN_2D_KERNEL_H_INCLUDED__
...
class GCN_2D_Kernel {
public:
	GCN_2D_Kernel(int nLevels, int max_nVertices, int nFeatures, int nHiddens, int nDepth, int max_Radius, double momentum_param) {
	...
	void computation_graph() {
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l == 0) {
				...
				level[l] -> neighbor_X = new RisiLayer2D* [max_nVertices];
				level[l] -> part1_X = new MatVecMul* [max_nVertices];
				level[l] -> part2_X = new MatVecMul* [max_nVertices];

				level[l] -> neighbor_Y = new RisiLayer2D* [max_nVertices];
				...
				for (int v = 0; v < max_nVertices; ++v) {
					...
					level[l] -> neighbor_X[v] = new RisiLayer2D(nHiddens);
					level[l] -> part2_X[v] = new MatVecMul(level[l] -> W2, level[l] -> neighbor_X[v]);
					level[l] -> sum_X[v] = new Add(level[l] -> part1_X[v], level[l] -> part2_X[v]);
					level[l] -> hidden_X[v] = new Softmax(level[l] -> sum_X[v]);

					level[l] -> part1_Y[v] = new MatVecMul(level[l] -> W1, feature_Y[v]);
					level[l] -> neighbor_Y[v] = new RisiLayer2D(nHiddens);
					level[l] -> part2_Y[v] = new MatVecMul(level[l] -> W2, level[l] -> neighbor_Y[v]);
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l == 0) {
				...
				for (int v = 0; v < molecule_X -> nVertices; ++v) {
					...
					graph -> add(level[l] -> neighbor_X[v], RISILAYER2D);
				...
				for (int v = 0; v < molecule_Y -> nVertices; ++v) {
					...
					graph -> add(level[l] -> neighbor_Y[v], RISILAYER2D);
	...
	double getLoss(int nBatch, DenseGraph **molecule_X, DenseGraph **molecule_Y, double *target) {
	...
	pair < double, double > BatchLearn(int nBatch, DenseGraph **molecule_X, DenseGraph **molecule_Y, double *target, double learning_rate) {
		assert(nBatch > 0);
	...
	pair<double, double> BatchLearn(int nBatch, DenseGraph **molecule_X, DenseGraph **molecule_Y, double *target, int nIterations, double learning_rate, double epsilon) {
		assert(nBatch > 0);
		...
		for (int iter = 0; iter < nIterations; ++iter) {
			...
			double loss = getLoss(nBatch, molecule_X, molecule_Y, target);
	...
	pair<double, double> Learn(DenseGraph *molecule_X, DenseGraph *molecule_Y, double target, int nIterations, double learning_rate, double epsilon) {
	...
	double Predict(DenseGraph *molecule_X, DenseGraph *molecule_Y) {
		complete_computation_graph(molecule_X, molecule_Y);
	...
	vector<double> Feature(DenseGraph *molecule_X, DenseGraph *molecule_Y) {
		complete_computation_graph(molecule_X, molecule_Y);
