// Framework: GraphFlow
// Class: Second-order Unrestricted Steerable Message Passing (version 2)
// Author: Machine Learning Group of UChicago
// Institution: Department of Computer Science, The University of Chicago
// Copyright 2017 (c) UChicago. All rights reserved.

#ifndef __Unrestricted_SMP_2D_VER2_H_INCLUDED__
#define __Unrestricted_SMP_2D_VER2_H_INCLUDED__
...
class Unrestricted_SMP_2D_ver2 {
public:
	Unrestricted_SMP_2D_ver2(int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, double momentum_param) {
	...
	Unrestricted_SMP_2D_ver2(int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, double momentum_param, bool has_WL_ordering) {
		this -> max_nVertices = max_nVertices;
		this -> nLevels = nLevels;
		this -> nChanels = nChanels;
		this -> nFeatures = nFeatures;
	...
	void computation_graph() {
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l > 0) {
				...
				level[l] -> W = new Tensor4D* [max_nVertices + 1];
				level[l] -> b = new Vector* [max_nVertices + 1];

				for (int size = 1; size <= max_nVertices; ++size) {
					level[l] -> W[size] = new Tensor4D(size, size, prevC, C);
	...
	void complete_computation_graph(DenseGraph *molecule) {
		...
		for (int l = 1; l <= nLevels; ++l) {
			for (int size = 1; size <= max_nVertices; ++size) {
				graph -> add(level[l] -> W[size], TENSOR4D);
