// Framework: GraphFlow
// Class: Generalized Steerable Convolutional Networks (Second-order representation)
// Author: Machine Learning Group of UChicago
// Institution: Department of Computer Science, The University of Chicago
// Copyright 2017 (c) UChicago. All rights reserved.

#ifndef __GCN_2D_DISTANCE_H_INCLUDED__
#define __GCN_2D_DISTANCE_H_INCLUDED__
...
class GCN_2D_Distance {
public:
	GCN_2D_Distance(int nLevels, int max_nVertices, int nFeatures, int nHiddens, int nDepth, int max_Radius, double momentum_param) {
	...
	void computation_graph() {
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l == 0) {
				...
				chanel_vertex[l] -> neighbor = new RisiLayer2D* [max_nVertices];
				...
				for (int v = 0; v < max_nVertices; ++v) {
					...
					chanel_vertex[l] -> neighbor[v] = new RisiLayer2D(nHiddens);
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l == 0) {
				...
				chanel_distance[l] -> neighbor = new RisiLayer2D* [max_nVertices];
				...
				for (int v = 0; v < max_nVertices; ++v) {
					...
					chanel_distance[l] -> neighbor[v] = new RisiLayer2D(nHiddens);
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l == 0) {
				...
				for (int v = 0; v < molecule -> nVertices; ++v) {
					...
					graph -> add(chanel_vertex[l] -> neighbor[v], RISILAYER2D);
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l == 0) {
				...
				for (int v = 0; v < molecule -> nVertices; ++v) {
					...
					graph -> add(chanel_distance[l] -> neighbor[v], RISILAYER2D);
