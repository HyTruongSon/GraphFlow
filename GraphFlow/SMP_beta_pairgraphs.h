// Framework: GraphFlow
// Class: Second-order Steerable Message Passing (Beta version + Pair graphs)
// Author: Machine Learning Group of UChicago
// Institution: Department of Computer Science, The University of Chicago
// Copyright 2017 (c) UChicago. All rights reserved.

#ifndef __SMP_BETA_PAIRGRAPHS_H_INCLUDED__
#define __SMP_BETA_PAIRGRAPHS_H_INCLUDED__
...
public:
	...
	SMP_beta_pairgraphs(int max_nVertices_1, int max_nVertices_2, int nLevels, int nChanels, int nFeatures_1, int nFeatures_2) {
	...
	SMP_beta_pairgraphs(bool use_coulomb, int max_nVertices_1, int max_nVertices_2, int nLevels, int nChanels, int nFeatures_1, int nFeatures_2) {
	...
	void init_multi_threads(int nThreads) {
		...
		instance = new SMP_beta_pairgraphs* [this -> nThreads];
		for (int i = 0; i < nThreads; ++i) {
			instance[i] = new SMP_beta_pairgraphs(use_coulomb, max_nVertices_1, max_nVertices_2, nLevels, nChanels, nFeatures_1,  nFeatures_2);
		}
	...
	void computation_graph_(
		...
		SMP_beta_pairgraphs::Level **level, Matrix **feature, ShrinkTensor ***shrinked, LeakyReLU ***vertex_feature, SumVectors **level_feature,
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l > 0) {
				...
				for (int v = 0; v < max_nVertices; ++v) {
					...
					level[l] -> contract[v] = new RisiContraction_18(max_nVertices, max_nVertices, nContractions * prevC);
					level[l] -> adj[v] = new Matrix(max_nVertices, max_nVertices);
					level[l] -> reshape2D[v] = new Reshape2D(max_nVertices * max_nVertices, nContractions * prevC);
					level[l] -> represent[v] = new MatMul(max_nVertices * max_nVertices, nContractions * prevC, nContractions * prevC, C);
					level[l] -> reshape3D[v] = new Reshape3D(max_nVertices, max_nVertices, C);
	...
	void init_receptive_field_permutation_matrix_reduced_adj(DenseGraph *molecule, SMP_beta_pairgraphs::Level **level, int **shortest_paths, int **adj) {
	...
	void complete_computation_graph_(DenseGraph *molecule, SMP_beta_pairgraphs::Level **level, Matrix *H, Matrix **feature, int **shortest_paths, 
	...
	static void compute_gradient_job(SMP_beta_pairgraphs *instance, 
	...
	static void predict_job(SMP_beta_pairgraphs *instance, DenseGraph *molecule_1, DenseGraph *molecule_2, double *predict, int position) {
