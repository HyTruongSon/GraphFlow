// Framework: GraphFlow
// Class: Generalized Steerable Convolutional Networks (Second-order representation) and Gated Recurrent Unit
// Author: Machine Learning Group of UChicago
// Institution: Department of Computer Science, The University of Chicago
// Copyright 2017 (c) UChicago. All rights reserved.

#ifndef __GRU_GCN_2D_H_INCLUDED__
#define __GRU_GCN_2D_H_INCLUDED__
...
class GRU_GCN_2D {
public:
	GRU_GCN_2D(int nLevels, int max_nVertices, int nFeatures, int nHiddens, int nDepth, int max_Radius, double momentum_param) {
	...
	void computation_graph() {
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l == 0) {
				...
				// Representation
				level[l] -> neighbor = new RisiLayer2D* [max_nVertices];
				...
				for (int v = 0; v < max_nVertices; ++v) {
					// Representation
					level[l] -> neighbor[v] = new RisiLayer2D(nHiddens);
		...
		for (int l = 0; l <= nLevels; ++l) {
			if (l == 0) {
				...
				for (int v = 0; v < molecule -> nVertices; ++v) {
					...
					graph -> add(level[l] -> neighbor[v], RISILAYER2D);
