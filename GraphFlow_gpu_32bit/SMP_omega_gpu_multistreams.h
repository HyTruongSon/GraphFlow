// Framework: GraphFlow
// Class: Second-order Steerable Message Passing (Omega version with GPU and multi-streams)
// Author: Machine Learning Group of UChicago
// Institution: Department of Computer Science, The University of Chicago
// Copyright 2017 (c) UChicago. All rights reserved.

#ifndef __SMP_OMEGA_GPU_MULTISTREAMS_H_INCLUDED__
#define __SMP_OMEGA_GPU_MULTISTREAMS_H_INCLUDED__
...
class SMP_omega_gpu_multistreams {
public:
	SMP_omega_gpu_multistreams(int max_nVertices, int max_receptive_field, int nLevels, int nChanels, int nFeatures, int nDepth) {
	...
	SMP_omega_gpu_multistreams(int max_nVertices, int max_receptive_field, int nLevels, int nChanels, int nFeatures, int nDepth, bool has_WL_ordering) {
		assert(max_receptive_field <= max_nVertices);
	...
	SMP_omega_gpu_multistreams(bool use_coulomb, int max_nVertices, int max_receptive_field, int nLevels, int nChanels, int nFeatures, int nDepth) {
		assert(max_receptive_field <= max_nVertices);
	...
	SMP_omega_gpu_multistreams(bool use_coulomb, int max_nVertices, int max_receptive_field, int nLevels, int nChanels, int nFeatures, int nDepth, bool has_WL_ordering) {
		assert(max_receptive_field <= max_nVertices);
	...
	void init_multi_threads(int nThreads) {
		...
		instance = new SMP_omega_gpu_multistreams* [this -> nThreads];
		for (int i = 0; i < nThreads; ++i) {
			instance[i] = new SMP_omega_gpu_multistreams(use_coulomb, max_nVertices, max_receptive_field, nLevels, nChanels, nFeatures, nDepth, has_WL_ordering);
		}
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l > 0) {
				...
				for (int v = 0; v < max_nVertices; ++v) {
					...
					level[l] -> stack[v] = new StackTensor3D_thread(N, N, N, nChanels);
					level[l] -> contract[v] = new RisiContraction_18_gpu(N, nChanels);
					level[l] -> adj[v] = new Matrix(N, N);
					level[l] -> reshape2D[v] = new Reshape2D(N * N, nContractions * nChanels);
					level[l] -> represent[v] = new MatMul_gpu(N * N, nContractions * nChanels, nContractions * nChanels, nChanels);
	...
	static void compute_gradient_job(SMP_omega_gpu_multistreams *instance, cudaStream_t stream, DenseGraph *molecule, float target) {
	...
	static void predict_job(SMP_omega_gpu_multistreams *instance, cudaStream_t stream, DenseGraph *molecule, float *predict, int position) {
		instance -> complete_computation_graph(molecule);
	...
	SMP_omega_gpu_multistreams **instance;
