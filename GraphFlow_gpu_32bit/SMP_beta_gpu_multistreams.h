// Framework: GraphFlow
// Class: Second-order Steerable Message Passing (Beta version with GPU and multi-streams)
// Author: Machine Learning Group of UChicago
// Institution: Department of Computer Science, The University of Chicago
// Copyright 2017 (c) UChicago. All rights reserved.

#ifndef __SMP_BETA_GPU_MULTISTREAMS_H_INCLUDED__
#define __SMP_BETA_GPU_MULTISTREAMS_H_INCLUDED__
...
class SMP_beta_gpu_multistreams {
public:
	SMP_beta_gpu_multistreams(int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth) {
	...
	SMP_beta_gpu_multistreams(int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, bool has_WL_ordering) {
		this -> use_coulomb = false;
		this -> max_nVertices = max_nVertices;
		this -> nLevels = nLevels;
		this -> nChanels = nChanels;
	...
	SMP_beta_gpu_multistreams(bool use_coulomb, int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth) {
	...
	SMP_beta_gpu_multistreams(bool use_coulomb, int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, bool has_WL_ordering) {
		this -> use_coulomb = use_coulomb;
		this -> max_nVertices = max_nVertices;
		this -> nLevels = nLevels;
		this -> nChanels = nChanels;
	...
	void init_multi_threads(int nThreads) {
		...
		instance = new SMP_beta_gpu_multistreams* [this -> nThreads];
		for (int i = 0; i < nThreads; ++i) {
			instance[i] = new SMP_beta_gpu_multistreams(use_coulomb, max_nVertices, nLevels, nChanels, nFeatures, nDepth, has_WL_ordering);
		}

		// Initialize multi-threaded jobs (CPU)
		job = new std::thread [this -> nThreads];

		// Initialize multi-streams (GPU)
		streams = new cudaStream_t [this -> nThreads];
	...
	void computation_graph() {
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l > 0) {
				...
				// For computations
				level[l] -> f = new LeakyReLU3D* [max_nVertices];
				level[l] -> stack = new StackTensor3D_thread* [max_nVertices];
				level[l] -> contract = new RisiContraction_18_gpu* [max_nVertices];
				level[l] -> adj = new Matrix* [max_nVertices];
				level[l] -> reshape2D = new Reshape2D* [max_nVertices];
				level[l] -> represent = new MatMul_gpu* [max_nVertices];
				...
				for (int v = 0; v < max_nVertices; ++v) {
					...
					level[l] -> stack[v] = new StackTensor3D_thread(max_nVertices, max_nVertices, max_nVertices, nChanels);
					level[l] -> contract[v] = new RisiContraction_18_gpu(max_nVertices, nChanels);
					level[l] -> adj[v] = new Matrix(max_nVertices, max_nVertices);
					level[l] -> reshape2D[v] = new Reshape2D(max_nVertices * max_nVertices, nContractions * nChanels);
					level[l] -> represent[v] = new MatMul_gpu(max_nVertices * max_nVertices, nContractions * nChanels, nContractions * nChanels, nChanels);
					level[l] -> reshape3D[v] = new Reshape3D(max_nVertices, max_nVertices, nChanels);
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l > 0) {
				for (int v = 0; v < molecule -> nVertices; ++v) {
					...
					// Stacking
					level[l] -> stack[v] -> setParameter(size, size, size, nChanels);
					level[l] -> stack[v] -> clear();
					...
					for (int i = 0; i < size; ++i) {
						...
						level[l] -> stack[v] -> add_tensor(level[l] -> quadratic[v][w]);
					}

					graph -> add(level[l] -> stack[v], STACKTENSOR3D_THREAD);

					// Contraction
					level[l] -> contract[v] -> setParameter(level[l] -> stack[v], level[l] -> adj[v]);
					graph -> add(level[l] -> contract[v], RISICONTRACTION_18_GPU);
					...
					graph -> add(level[l] -> represent[v], MATMUL_GPU);
	...
	static void compute_gradient_job(SMP_beta_gpu_multistreams *instance, cudaStream_t stream, DenseGraph *molecule, float target) {
		instance -> complete_computation_graph(molecule);
		instance -> target -> value[0] = target;

		for (int l = 1; l <= instance -> nLevels; ++l) {
			for (int v = 0; v < molecule -> nVertices; ++v) {
				instance -> level[l] -> contract[v] -> set_gpu_stream(stream);
				instance -> level[l] -> represent[v] -> set_gpu_stream(stream);
			}
		}
	...
	void Threaded_BatchLearn(int nBatch, DenseGraph **molecule, float *target, float learning_rate) {
	...
	static void predict_job(SMP_beta_gpu_multistreams *instance, cudaStream_t stream, DenseGraph *molecule, float *predict, int position) {
		instance -> complete_computation_graph(molecule);
	...
	void Threaded_Predict(int nBatch, DenseGraph **molecule, float *predict) {
		assert(multi_threaded == true);
		assert(nThreads > 1);
	...
	// Instances for multi-threads (CPU)
	SMP_beta_gpu_multistreams **instance;

	// Multi-threaded jobs (CPU)
	std::thread *job;
	...
	struct Level {
		...
		// Tensor stacking
		StackTensor3D_thread **stack;

		// RisiContraction_18
		RisiContraction_18_gpu **contract;

		// Cut number of chanels 
		Reshape2D **reshape2D;
		MatMul_gpu **represent;
		Reshape3D **reshape3D;
