// Framework: GraphFlow
// Class: Second-order Steerable Message Passing (omega version with GPU)
// Author: Machine Learning Group of UChicago
// Institution: Department of Computer Science, The University of Chicago
// Copyright 2017 (c) UChicago. All rights reserved.

#ifndef __SMP_OMEGA_GPU_H_INCLUDED__
#define __SMP_OMEGA_GPU_H_INCLUDED__
...
class SMP_omega_gpu {
public:
	SMP_omega_gpu(int max_nVertices, int max_receptive_field, int nLevels, int nChanels, int nFeatures, int nDepth) {
	...
	SMP_omega_gpu(int max_nVertices, int max_receptive_field, int nLevels, int nChanels, int nFeatures, int nDepth, bool has_WL_ordering) {
		assert(max_receptive_field <= max_nVertices);
	...
	SMP_omega_gpu(bool use_coulomb, int max_nVertices, int max_receptive_field, int nLevels, int nChanels, int nFeatures, int nDepth) {
		assert(max_receptive_field <= max_nVertices);
	...
	SMP_omega_gpu(bool use_coulomb, int max_nVertices, int max_receptive_field, int nLevels, int nChanels, int nFeatures, int nDepth, bool has_WL_ordering) {
		assert(max_receptive_field <= max_nVertices);
	...
	void init_multi_threads(int nThreads) {
		...
		instance = new SMP_omega_gpu* [this -> nThreads];
		for (int i = 0; i < nThreads; ++i) {
			instance[i] = new SMP_omega_gpu(use_coulomb, max_nVertices, max_receptive_field, nLevels, nChanels, nFeatures, nDepth, has_WL_ordering);
		}
	...
	static void compute_gradient_job(SMP_omega_gpu *instance, DenseGraph *molecule, float target) {
	...
	static void predict_job(SMP_omega_gpu *instance, DenseGraph *molecule, float *predict, int position) {
		instance -> complete_computation_graph(molecule);
		instance -> graph -> forward();
