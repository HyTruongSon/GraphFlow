// Framework: GraphFlow
// Class: Second-order Steerable Message Passing (Gamma version)
// Author: Machine Learning Group of UChicago
// Institution: Department of Computer Science, The University of Chicago
// Copyright 2017 (c) UChicago. All rights reserved.

#ifndef __SMP_GAMMA_H_INCLUDED__
#define __SMP_GAMMA_H_INCLUDED__
...
class SMP_gamma {
public:
	SMP_gamma(int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth) {
	...
	SMP_gamma(int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, bool has_WL_ordering) {
		this -> use_coulomb = false;
		this -> max_nVertices = max_nVertices;
		this -> nLevels = nLevels;
		this -> nChanels = nChanels;
	...
	SMP_gamma(bool use_coulomb, int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth) {
	...
	SMP_gamma(bool use_coulomb, int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, bool has_WL_ordering) {
		this -> use_coulomb = use_coulomb;
		this -> max_nVertices = max_nVertices;
		this -> nLevels = nLevels;
		this -> nChanels = nChanels;
	...
	void init_multi_threads(int nThreads) {
		...
		instance = new SMP_gamma* [this -> nThreads];
		for (int i = 0; i < nThreads; ++i) {
			instance[i] = new SMP_gamma(use_coulomb, max_nVertices, nLevels, nChanels, nFeatures, nDepth, has_WL_ordering);
		}
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l > 0) {
				...
				for (int v = 0; v < max_nVertices; ++v) {
					...
					level[l] -> contract[v] = new RisiContraction_4(max_nVertices, max_nVertices, nContractions * nChanels);
					level[l] -> reshape2D[v] = new Reshape2D(max_nVertices * max_nVertices, nContractions * nChanels);
					level[l] -> represent[v] = new MatMul(max_nVertices * max_nVertices, nContractions * nChanels, nContractions * nChanels, nChanels);
	...
	void complete_computation_graph(DenseGraph *molecule) {
		...
		// Initialize the receptive fields, permutation matrices
	...
	static void compute_gradient_job(SMP_gamma *instance, DenseGraph *molecule, float target) {
	...
	static void predict_job(SMP_gamma *instance, DenseGraph *molecule, float *predict, int position) {
		instance -> complete_computation_graph(molecule);
		instance -> graph -> forward();
