// Framework: GraphFlow
// Class: Second-order Steerable Message Passing (Beta version with GPU)
// Author: Machine Learning Group of UChicago
// Institution: Department of Computer Science, The University of Chicago
// Copyright 2017 (c) UChicago. All rights reserved.

#ifndef __SMP_BETA_GPU_H_INCLUDED__
#define __SMP_BETA_GPU_H_INCLUDED__
...
class SMP_beta_gpu {
public:
	SMP_beta_gpu(int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth) {
	...
	SMP_beta_gpu(int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, bool has_WL_ordering) {
		this -> use_coulomb = false;
		this -> max_nVertices = max_nVertices;
		this -> nLevels = nLevels;
		this -> nChanels = nChanels;
	...
	SMP_beta_gpu(bool use_coulomb, int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth) {
	...
	SMP_beta_gpu(bool use_coulomb, int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, bool has_WL_ordering) {
		this -> use_coulomb = use_coulomb;
		this -> max_nVertices = max_nVertices;
		this -> nLevels = nLevels;
		this -> nChanels = nChanels;
	...
	void init_multi_threads(int nThreads) {
		...
		instance = new SMP_beta_gpu* [this -> nThreads];
		for (int i = 0; i < nThreads; ++i) {
			instance[i] = new SMP_beta_gpu(use_coulomb, max_nVertices, nLevels, nChanels, nFeatures, nDepth, has_WL_ordering);
		}
	...
	static void compute_gradient_job(SMP_beta_gpu *instance, DenseGraph *molecule, float target) {
	...
	static void predict_job(SMP_beta_gpu *instance, DenseGraph *molecule, float *predict, int position) {
		instance -> complete_computation_graph(molecule);
		instance -> graph -> forward();
