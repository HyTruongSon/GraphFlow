...
__global__ void Matrix_Multiplication_GPU(float *A, float *B, float *C, int A_nRows, int A_nColumns, int B_nRows, int B_nColumns) {
	__shared__ float shared_A[MATMUL_GPU_BLOCK_SIZE][MATMUL_GPU_BLOCK_SIZE];
	__shared__ float shared_B[MATMUL_GPU_BLOCK_SIZE][MATMUL_GPU_BLOCK_SIZE];
...
__global__ void MatMul_backward_first(float *A_gradient, float *B, float *C_gradient, int A_nRows, int A_nColumns, int B_nRows, int B_nColumns) {
...
__global__ void MatMul_backward_second(float *A, float *B_gradient, float *C_gradient, int A_nRows, int A_nColumns, int B_nRows, int B_nColumns) {
	__shared__ int C_nColumns;
...
public:
	MatMul_gpu(int max_first_nRows, int max_first_nColumns, int max_second_nRows, int max_second_nColumns) : Matrix(max_first_nRows, max_second_nColumns) {
		...
		size_first = max_first_nRows * max_first_nColumns * sizeof(float);
		size_second = max_second_nRows * max_second_nColumns * sizeof(float);
		size_this = size * sizeof(float);
		...
		temp = new float [max(max_first_nRows * max_first_nColumns, max_second_nRows * max_second_nColumns)];
	...
	MatMul_gpu(Matrix *first, Matrix *second) : Matrix(first -> nRows, second -> nColumns) {
		...
		size_first = first -> nRows * first -> nColumns * sizeof(float);
		size_second = second -> nRows * second -> nColumns * sizeof(float);
		size_this = size * sizeof(float);
		...
		// Memory for the DEPRECATED backward
		temp = new float [max(first -> size, second -> size)];
