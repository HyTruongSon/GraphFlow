...
__global__ void DEPRECATED_RisiContraction_18_forward_job(float *tensor, float *adj, float *value, int N, int nChanels) {
	__shared__ int d;
	__shared__ int e;
	__shared__ float adj_value;
	...
	partition_size = ceil((float)(length) / (float)(blockDim.x)); 
	nContractions = 18;
...
__global__ void DEPRECATED_RisiContraction_18_backward_job(float *tensor_gradient, float *adj, float *gradient, int N, int nChanels) {
	__shared__ int d;
	__shared__ int e;
	__shared__ float adj_value;
	__shared__ int length;
...
public:
	...
	RisiContraction_18_gpu(int max_N, int max_nChanels) : Tensor3D(max_N, max_N, nContractions * max_nChanels) {
		...
		tensor_size = max_N * max_N * max_N * max_nChanels * sizeof(float);
		...
		adj_size = max_N * max_N * sizeof(float);

		err = cudaMalloc( &device_adj_value, adj_size);
		assert(err == cudaSuccess);

		// GPU device - For this object
		this_size = max_N * max_N * max_nChanels * nContractions * sizeof(float);
	...
	RisiContraction_18_gpu(Tensor4D *tensor, Matrix *adj) : Tensor3D(tensor -> nRows, tensor -> nRows, nContractions * tensor -> nChanels2) {
		...
		// GPU device - Tensor of 4 dimensions
		tensor_size = N * N * N * nChanels * sizeof(float);
		...
		// GPU device - The reduced adjacency matrix
		adj_size = N * N * sizeof(float);

		err = cudaMalloc((void **) &device_adj_value, adj_size);
		assert(err == cudaSuccess);

		// GPU device - For this object
		this_size = N * N * nChanels * nContractions * sizeof(float);
	...
	// GPU device - Tensor of 4 dimensions
	float *device_tensor_value;
	float *device_tensor_gradient;

	// GPU device - The reduced adjacency matrix
	float *device_adj_value;
