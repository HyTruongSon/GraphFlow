...
string model_fn = "GRU_GCN_3D-model.dat";

GRU_GCN_3D train_network(nLevels, max_nVertices, nFeatures, nHiddens, nDepth, max_Radius, momentum_param);
GRU_GCN_3D test_network(nLevels, max_nVertices, nFeatures, nHiddens, nDepth, max_Radius, momentum_param);
...
int main(int argc, char **argv) {
	...
	for (int j = 0; j < nEpochs; ++j) {
		pair < double, double > info = train_network.BatchLearn(nMolecules, graphs, targets, learning_rate);
