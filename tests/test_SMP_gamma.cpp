...
// To compile: g++ -std=c++11 -pthread test_SMP_gamma.cpp
...
#include "../GraphFlow_32bit/SMP_gamma.h"
...
string model_fn = "SMP_gamma-model.dat";

SMP_gamma train_network(max_nVertices, nLevels, nChanels, nFeatures, nDepth);
SMP_gamma test_network(max_nVertices, nLevels, nChanels, nFeatures, nDepth);
...
int main(int argc, char **argv) {
	...
	for (int j = 0; j < nEpochs; ++j) {
		...
		for (int i = 0; i < nMolecules; ++i) {
			...
			float predict = train_network.Predict(molecule[i] -> graph);
