...
// To compile: g++ -std=c++11 -pthread test_SMP_theta.cpp
...
string model_fn = "SMP_theta-model.dat";

SMP_theta train_network(max_nVertices, max_receptive_field, nLevels, nChanels, nFeatures, nDepth);
SMP_theta test_network(max_nVertices, max_receptive_field, nLevels, nChanels, nFeatures, nDepth);
...
int main(int argc, char **argv) {
	...
	for (int j = 0; j < nEpochs; ++j) {
		...
		for (int i = 0; i < nMolecules; ++i) {
			...
			double predict = train_network.Predict(molecule[i] -> graph);
