...
const int max_nVertices = 6;
const int nFeatures = 4;
const int nNeighbors = 5;
const int nDepth = 3;
const int nChanels1 = 16;
const int nChanels2 = 8;
const int nDense = 128;
...
string model_fn = "LCNN-model.dat";

LCNN train_network(max_nVertices, nFeatures, nNeighbors, nDepth, nChanels1, nChanels2, nDense, momentum_param);
LCNN test_network(max_nVertices, nFeatures, nNeighbors, nDepth, nChanels1, nChanels2, nDense, momentum_param);
...
struct Molecule {
	...
	void build() {
		for (int i = 0; i < edge.size(); ++i) {
			...
			graph -> distance[u][v] = 1.0;
			graph -> distance[v][u] = 1.0;
		}
