...
#ifndef __SMP_BETA_PHYSICS_H_INCLUDED__
#define __SMP_BETA_PHYSICS_H_INCLUDED__
...
class SMP_beta_physics {
public:
	SMP_beta_physics(int max_nVertices, int nLevels, int nChanels, int nFeatures) {
	...
	SMP_beta_physics(bool use_coulomb, int max_nVertices, int nLevels, int nChanels, int nFeatures) {
	...
	void init_multi_threads(int nThreads) {
		...
		instance = new SMP_beta_physics* [this -> nThreads];
		for (int i = 0; i < nThreads; ++i) {
			instance[i] = new SMP_beta_physics(use_coulomb, max_nVertices, nLevels, nChanels, nFeatures);
		}
	...
	static void compute_gradient_job(SMP_beta_physics *instance, DenseGraph *molecule, float target) {
	...
	static void predict_job(SMP_beta_physics *instance, DenseGraph *molecule, float *predict, int position) {
		instance -> complete_computation_graph(molecule);
		instance -> graph -> forward();
