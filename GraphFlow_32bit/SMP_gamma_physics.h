...
#ifndef __SMP_GAMMA_PHYSICS_H_INCLUDED__
#define __SMP_GAMMA_PHYSICS_H_INCLUDED__
...
class SMP_gamma_physics {
public:
	SMP_gamma_physics(int max_nVertices, int max_receptive_field, int nLevels, int nChanels, int nFeatures) {
	...
	SMP_gamma_physics(bool use_coulomb, int max_nVertices, int max_receptive_field, int nLevels, int nChanels, int nFeatures) {
	...
	void init_multi_threads(int nThreads) {
		...
		instance = new SMP_gamma_physics* [this -> nThreads];
		for (int i = 0; i < nThreads; ++i) {
			instance[i] = new SMP_gamma_physics(use_coulomb, max_nVertices, max_receptive_field, nLevels, nChanels, nFeatures);
		}
	...
	static void compute_gradient_job(SMP_gamma_physics *instance, DenseGraph *molecule, float target) {
	...
	static void predict_job(SMP_gamma_physics *instance, DenseGraph *molecule, float *predict, int position) {
		instance -> complete_computation_graph(molecule);
		instance -> graph -> forward();
