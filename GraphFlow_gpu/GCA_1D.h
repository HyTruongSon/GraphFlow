...
class GCA_1D {
public:
	GCA_1D(int nLevels, int max_nVertices, int nFeatures, int nHiddens, int nDepth, int max_Radius, double momentum_param) {
	...
	double getLoss(int nBatch, DenseGraph **molecule) {
	...
	pair < double, double > BatchLearn(int nBatch, DenseGraph **molecule, double learning_rate) {
		assert(nBatch > 0);
	...
	pair<double, double> BatchLearn(int nBatch, DenseGraph **molecule, int nIterations, double learning_rate, double epsilon) {
		assert(nBatch > 0);
		assert(molecule[0] -> nVertices <= max_nVertices);
		assert(molecule[0] -> nFeatures == nFeatures);
		...
		for (int iter = 0; iter < nIterations; ++iter) {
			...
			double loss = getLoss(nBatch, molecule);
	...
	pair<double, double> Learn(DenseGraph *molecule, int nIterations, double learning_rate, double epsilon) {
	...
	void Predict(DenseGraph *molecule, double **mat) {
		complete_computation_graph(molecule);
