// Framework: GraphFlow
// Class: Second-order Steerable Message Passing (version 5)
// Author: Machine Learning Group of UChicago
// Institution: Department of Computer Science, The University of Chicago
// Copyright 2017 (c) UChicago. All rights reserved.

#ifndef __SMP_2D_VER5_H_INCLUDED__
#define __SMP_2D_VER5_H_INCLUDED__
...
class SMP_2D_ver5 {
public:
	SMP_2D_ver5(int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, double momentum_param) {
	...
	SMP_2D_ver5(int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, double momentum_param, bool has_WL_ordering) {
		this -> max_nVertices = max_nVertices;
		this -> nLevels = nLevels;
		this -> nChanels = nChanels;
		this -> nFeatures = nFeatures;
	...
	void computation_graph() {
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l > 0) {
				...
				level[l] -> K = new Matrix(nChanels, 2 * nChanels);
				...
				for (int v = 0; v < max_nVertices; ++v) {
					...
					level[l] -> affine_eye[v] = new TensorMul(max_nVertices, max_nVertices, nChanels);
					level[l] -> affine_one[v] = new TensorMul(max_nVertices, max_nVertices, nChanels);
					level[l] -> concat[v] = new Tensor3DConcat(max_nVertices, max_nVertices, 2 * nChanels);
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l > 0) {
				for (int v = 0; v < molecule -> nVertices; ++v) {
					...
					level[l] -> represent[v] -> setParameter(level[l] -> K, level[l] -> concat[v]);
