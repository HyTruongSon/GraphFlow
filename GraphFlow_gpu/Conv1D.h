...
public:
	...
	: Matrix(ceil(double(input -> nRows + 2 * pad - filter -> nRows + 1) / double(stride)), filter -> nDepth) {
				...
				for (int c1 = 0; c1 < nInputChanels; ++c1) {
					for (int v = 0; v < filter -> nRows; ++v) {
						if ((i + v >= pad) && (i + v < input -> nRows + pad)) {
							double input_value = input -> value[input -> index(i + v - pad, c1)];
							double filter_value = filter -> value[filter -> index(v, c1, c2)];
	...
	void backward() {
		...
		while (i < input -> nRows + 2 * pad - filter -> nRows + 1) {
			for (int c2 = 0; c2 < nOutputChanels; ++c2) {
				double grad = gradient[index(j, c2)];
				for (int c1 = 0; c1 < nInputChanels; ++c1) {
					for (int v = 0; v < filter -> nRows; ++v) {
						if ((i + v >= pad) && (i + v < input -> nRows + pad)) {
							double input_value = input -> value[input -> index(i + v - pad, c1)];
