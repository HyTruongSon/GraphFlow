// Framework: GraphFlow
// Class: Second-order Steerable Message Passing (version 4)
// Author: Machine Learning Group of UChicago
// Institution: Department of Computer Science, The University of Chicago
// Copyright 2017 (c) UChicago. All rights reserved.

#ifndef __SMP_2D_VER4_CLASSIFICATION_H_INCLUDED__
#define __SMP_2D_VER4_CLASSIFICATION_H_INCLUDED__
...
class SMP_2D_ver4_classification {
public:
	SMP_2D_ver4_classification(int nClass, int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, double momentum_param) {
		this -> nClass = nClass;
	...
	SMP_2D_ver4_classification(int nClass, int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, double momentum_param, bool has_WL_ordering) {
		this -> nClass = nClass;
		this -> max_nVertices = max_nVertices;
		this -> nLevels = nLevels;
		this -> nChanels = nChanels;
	...
	void computation_graph() {
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l > 0) {
				// Double the number of chanels in this level
				...
				for (int size = 1; size <= max_nVertices; ++size) {
					level[l] -> lambda1[size] = new Vector(prevC);
					level[l] -> lambda2[size] = new Vector(prevC);

					level[l] -> W_eye[size] = new VectorBroadcastMat(size, size, prevC);
					level[l] -> W_one[size] = new VectorBroadcastMat(size, size, prevC);
				...
				level[l] -> affine_eye = new TensorMul* [max_nVertices];
				level[l] -> affine_one = new TensorMul* [max_nVertices];
				level[l] -> concat = new Tensor3DConcat* [max_nVertices];
				...
				for (int v = 0; v < max_nVertices; ++v) {
					...
					level[l] -> affine_eye[v] = new TensorMul(max_nVertices, max_nVertices, prevC);
					level[l] -> affine_one[v] = new TensorMul(max_nVertices, max_nVertices, prevC);
					level[l] -> concat[v] = new Tensor3DConcat(max_nVertices, max_nVertices, C);
			...
			for (int v = 0; v < molecule -> nVertices; ++v) {
				...
				for (int i = 0; i < level[l] -> phi[v].size(); ++i) {
					...
					for (int j = 0; j < level[l] -> phi[v].size(); ++j) {
						...
						degree += level[l] -> adj[v] -> value[ind];
					}

					for (int j = 0; j < level[l] -> phi[v].size(); ++j) {
						int ind = level[l] -> adj[v] -> index(i, j);
						level[l] -> adj[v] -> value[ind] /= degree;
					}
				}
			}
		}
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l > 0) {
				...
				for (int v = 0; v < molecule -> nVertices; ++v) {
					...
					level[l] -> affine_eye[v] -> setParameter(level[l] -> W_eye[size], level[l] -> quadratic_plus_adj[v]);
					graph -> add(level[l] -> affine_eye[v], TENSORMUL);

					level[l] -> affine_one[v] -> setParameter(level[l] -> W_one[size], level[l] -> quadratic_plus_adj[v]);
					graph -> add(level[l] -> affine_one[v], TENSORMUL);

					// Tensor concatenation
					level[l] -> concat[v] -> setParameter(level[l] -> affine_eye[v], level[l] -> affine_one[v]);
					graph -> add(level[l] -> concat[v], TENSOR3DCONCAT);

					level[l] -> add[v] -> setParameter(level[l] -> b[size], level[l] -> concat[v]);
	...
	pair < double, double > Learn(DenseGraph *molecule, double target, int nIterations, double learning_rate, double epsilon) {
		...
		graph -> forward();
		double best_error = logl -> getLoss();
	...
	struct Level {
		...
		TensorMul **affine_eye;
		TensorMul **affine_one;

		// Tensor3D concatenation
		Tensor3DConcat **concat;
	...
	~SMP_2D_ver4_classification() {
		...
		for (int l = 0; l <= nLevels; ++l) {
			if (l == 0) {
				...
				delete[] level[l] -> affine_eye;
				delete[] level[l] -> affine_one;
				delete[] level[l] -> add;
				delete[] level[l] -> phi;
