// Framework: GraphFlow
// Class: Generalized Steerable Convolutional Networks (First-order representation) for Kernel Learning
// Author: Machine Learning Group of UChicago
// Institution: Department of Computer Science, The University of Chicago
// Copyright 2017 (c) UChicago. All rights reserved.

#ifndef __GCN_1D_KERNEL_H_INCLUDED__
#define __GCN_1D_KERNEL_H_INCLUDED__
...
class GCN_1D_Kernel {
public:
	GCN_1D_Kernel(int nLevels, int max_nVertices, int nFeatures, int nHiddens, int nDepth, int max_Radius, double momentum_param) {
	...
	void computation_graph() {
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l == 0) {
				...
				level[l] -> neighbor_X = new RisiLayer1D* [max_nVertices];
				level[l] -> part1_X = new MatVecMul* [max_nVertices];
				level[l] -> part2_X = new MatVecMul* [max_nVertices];

				level[l] -> neighbor_Y = new RisiLayer1D* [max_nVertices];
				...
				for (int v = 0; v < max_nVertices; ++v) {
					...
					level[l] -> neighbor_X[v] = new RisiLayer1D(nHiddens);
					...
					level[l] -> neighbor_Y[v] = new RisiLayer1D(nHiddens);
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l == 0) {
				...
				for (int v = 0; v < molecule_X -> nVertices; ++v) {
					...
					graph -> add(level[l] -> neighbor_X[v], RISILAYER1D);
				...
				for (int v = 0; v < molecule_Y -> nVertices; ++v) {
					...
					graph -> add(level[l] -> neighbor_Y[v], RISILAYER1D);
