...
#include "RisiContraction_18_gpu.h"
...
const int RISICONTRACTION_18_GPU 		= 100;
const int MATMUL_GPU 					= 101;
...
public:
	...
	void forward() {
		for (int i = 0; i < topology.size(); ++i) {
			...
			if (RISICONTRACTION_18_GPU == topology[i].second) {
				RisiContraction_18_gpu *obj = (RisiContraction_18_gpu *)(topology[i].first);
				obj -> forward();

				continue;
			}

			if (MATMUL_GPU == topology[i].second) {
				MatMul_gpu *obj = (MatMul_gpu *)(topology[i].first);
				obj -> forward();
	...
	void assign_init(Vector *V, double value) {
	...
	void uniform_init(Matrix *M) {
		...
		for (int row = 0; row < M -> nRows; ++row) {
			for (int column = 0; column < M -> nColumns; ++column) {
				...
				M -> value[index] = (double)(rand() % 10) / (10.0 * (M -> nRows));
	...
	void uniform_init(Vector *V) {
		int sign;
		for (int i = 0; i < V -> size; ++i) {
			V -> value[i] = (double)(rand() % 10) / (10.0 * V -> size);
			sign = rand() % 2;
			if (sign == 1) {
				V -> value[i] = - V -> value[i];
			}
	...
	double rand_uniform() {
		...
		return double(number) / double(RANGE);
	}
	...
	double rand_uniform(double radius) {
		return rand_uniform(-radius, radius);
	}

	void Xavier_init(Vector *V) {
		double radius = sqrt(3.0 / V -> size);
		for (int i = 0; i < V -> size; ++i) {
