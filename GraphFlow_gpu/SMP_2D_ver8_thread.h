// Framework: GraphFlow
// Class: Second-order Steerable Message Passing (version 8) Multi-threading
// Author: Machine Learning Group of UChicago
// Institution: Department of Computer Science, The University of Chicago
// Copyright 2017 (c) UChicago. All rights reserved.

#ifndef __SMP_2D_VER8_THREAD_H_INCLUDED__
#define __SMP_2D_VER8_THREAD_H_INCLUDED__
...
class SMP_2D_ver8_thread {
public:
	SMP_2D_ver8_thread(int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, double momentum_param) {
	...
	SMP_2D_ver8_thread(int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, double momentum_param, bool has_WL_ordering) {
		this -> use_coulomb = false;
		this -> max_nVertices = max_nVertices;
		this -> nLevels = nLevels;
		this -> nChanels = nChanels;
	...
	SMP_2D_ver8_thread(bool use_coulomb, int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, double momentum_param) {
	...
	SMP_2D_ver8_thread(bool use_coulomb, int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, double momentum_param, bool has_WL_ordering) {
		this -> use_coulomb = use_coulomb;
		this -> max_nVertices = max_nVertices;
		this -> nLevels = nLevels;
		this -> nChanels = nChanels;
	...
	void computation_graph() {
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l > 0) {
				...
				// For computations
				level[l] -> f = new LeakyReLU3D* [max_nVertices];
				level[l] -> contract = new RisiContraction_18_thread* [max_nVertices];
				...
				for (int v = 0; v < max_nVertices; ++v) {
					...
					level[l] -> contract[v] = new RisiContraction_18_thread(max_nVertices, max_nVertices, nContractions * nChanels);
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l > 0) {
				for (int v = 0; v < molecule -> nVertices; ++v) {
					...
					graph -> add(level[l] -> contract[v], RISICONTRACTION_18_THREAD);
	...
	static const int nContractions = RisiContraction_18_thread::nContractions;
	...
	struct Level {
		...
		RisiContraction_18_thread **contract;
