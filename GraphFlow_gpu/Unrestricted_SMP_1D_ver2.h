// Framework: GraphFlow
// Class: First-order Unrestricted Steerable Message Passing (version 2)
// Author: Machine Learning Group of UChicago
// Institution: Department of Computer Science, The University of Chicago
// Copyright 2017 (c) UChicago. All rights reserved.

#ifndef __Unrestricted_SMP_1D_VER2_H_INCLUDED__
#define __Unrestricted_SMP_1D_VER2_H_INCLUDED__
...
class Unrestricted_SMP_1D_ver2 {
public:
	Unrestricted_SMP_1D_ver2(int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, double momentum_param) {
	...
	Unrestricted_SMP_1D_ver2(int max_nVertices, int nLevels, int nChanels, int nFeatures, int nDepth, double momentum_param, bool has_WL_ordering) {
		this -> max_nVertices = max_nVertices;
		this -> nLevels = nLevels;
		this -> nChanels = nChanels;
		this -> nFeatures = nFeatures;
	...
	void computation_graph() {
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l > 0) {
				...
				// Unrestricted Steerable Filters and Bias
				level[l] -> W1 = new Matrix* [max_nVertices + 1];
				level[l] -> W2 = new Matrix* [max_nVertices + 1];
				level[l] -> b = new Vector* [max_nVertices + 1];

				for (int size = 1; size <= max_nVertices; ++size) {
					level[l] -> W1[size] = new Matrix(size, size);
					level[l] -> W2[size] = new Matrix(size, size);
		...
		for (int l = 1; l <= nLevels; ++l) {
			for (int size = 1; size <= max_nVertices; ++size) {
				sgd -> add(level[l] -> W1[size]);
				sgd -> add(level[l] -> W2[size]);
	...
	void complete_computation_graph(DenseGraph *molecule) {
		...
		for (int l = 1; l <= nLevels; ++l) {
			for (int size = 1; size <= max_nVertices; ++size) {
				graph -> add(level[l] -> W1[size], MATRIX);
				graph -> add(level[l] -> W2[size], MATRIX);
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l > 0) {
				...
				for (int v = 0; v < molecule -> nVertices; ++v) {
					...
					level[l] -> affine[v] -> setParameter(level[l] -> W1[size], level[l] -> sum[v]);
					graph -> add(level[l] -> affine[v], MATMUL);

					level[l] -> affine2[v] -> setParameter(level[l] -> W2[size], level[l] -> sum[v]);
	...
	struct Level {
		...
		// Unrestricted Steerable Filters
		Matrix **W1;
		Matrix **W2;
