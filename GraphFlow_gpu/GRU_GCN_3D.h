// Framework: GraphFlow
// Class: Generalized Steerable Convolutional Networks (Third-order representation) and Gated Recurrent Unit
// Author: Machine Learning Group of UChicago
// Institution: Department of Computer Science, The University of Chicago
// Copyright 2017 (c) UChicago. All rights reserved.

#ifndef __GRU_GCN_3D_H_INCLUDED__
#define __GRU_GCN_3D_H_INCLUDED__
...
class GRU_GCN_3D {
public:
	GRU_GCN_3D(int nLevels, int max_nVertices, int nFeatures, int nHiddens, int nDepth, int max_Radius, double momentum_param) {
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l == 0) {
				...
				for (int v = 0; v < max_nVertices; ++v) {
					...
					// Update gate
					level[l] -> update_part1[v] = new MatVecMul(W_z, level[l] -> pooling[v]);
					...
					// Reset gate
					level[l] -> reset_part1[v] = new MatVecMul(W_r, level[l] -> pooling[v]);
					...
					level[l] -> candidate_part1[v] = new MatVecMul(W_h, level[l] -> pooling[v]);
