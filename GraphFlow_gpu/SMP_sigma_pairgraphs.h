// Framework: GraphFlow
// Class: Second-order Steerable Message Passing (Sigma version + Pair graphs)
// Author: Machine Learning Group of UChicago
// Institution: Department of Computer Science, The University of Chicago
// Copyright 2017 (c) UChicago. All rights reserved.

#ifndef __SMP_SIGMA_PAIRGRAPHS_H_INCLUDED__
#define __SMP_SIGMA_PAIRGRAPHS_H_INCLUDED__
...
public:
	...
	struct Level {
		...
		RisiContraction_18_dropout **contract;
	...
	SMP_sigma_pairgraphs(int max_nVertices_1, int max_nVertices_2, int max_receptive_field, int nLevels, int nChanels, int nFeatures_1, int nFeatures_2, int nKept) {
	...
	SMP_sigma_pairgraphs(bool use_coulomb, int max_nVertices_1, int max_nVertices_2, int max_receptive_field, int nLevels, int nChanels, int nFeatures_1, int nFeatures_2, int nKept) {
		assert(max_receptive_field <= max_nVertices_1);
		assert(max_receptive_field <= max_nVertices_2);
		assert(nKept > 0);
		assert(nKept <= nContractions);
	...
	void init_multi_threads(int nThreads) {
		...
		instance = new SMP_sigma_pairgraphs* [this -> nThreads];
		for (int i = 0; i < nThreads; ++i) {
			instance[i] = new SMP_sigma_pairgraphs(use_coulomb, max_nVertices_1, max_nVertices_2, max_receptive_field, nLevels, nChanels, nFeatures_1, nFeatures_2, nKept);
		}
	...
	void computation_graph_(
		...
		SMP_sigma_pairgraphs::Level **level, Matrix **feature, ShrinkTensor ***shrinked, LeakyReLU ***vertex_feature, SumVectors **level_feature,
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l > 0) {
				...
				// For computations
				level[l] -> f = new LeakyReLU3D* [max_nVertices];
				level[l] -> contract = new RisiContraction_18_dropout* [max_nVertices];
				...
				for (int v = 0; v < max_nVertices; ++v) {
					...
					level[l] -> contract[v] = new RisiContraction_18_dropout(N, N, nContractions * prevC);
					...
					// Set the number of kept contractions
					level[l] -> contract[v] -> setContractions(nKept);
				}
	...
	void computation_graph() {
		...
		// Mapping from the input graph features into chanels
	...
	void init_receptive_field_permutation_matrix_reduced_adj(DenseGraph *molecule, SMP_sigma_pairgraphs::Level **level, int **shortest_paths, int **adj) {
	...
	void complete_computation_graph_(DenseGraph *molecule, SMP_sigma_pairgraphs::Level **level, Matrix *H, Matrix **feature, int **shortest_paths, 
		...
		for (int l = 0; l <= nLevels; ++l) {
			...
			if (l > 0) {
				...
				for (int v = 0; v < molecule -> nVertices; ++v) {
					...
					// Set the training mode or testing mode
					level[l] -> contract[v] -> setMode(mode);
					...
					graph -> add(level[l] -> contract[v], RISICONTRACTION_18_DROPOUT);
	...
	void complete_computation_graph(DenseGraph *molecule_1, DenseGraph *molecule_2) {
		...
		init_receptive_field_permutation_matrix_reduced_adj(molecule_1, level_1, shortest_paths_1, adj_1);
		init_receptive_field_permutation_matrix_reduced_adj(molecule_2, level_2, shortest_paths_2, adj_2);
	...
	static void compute_gradient_job(SMP_sigma_pairgraphs *instance, 
	...
	static void predict_job(SMP_sigma_pairgraphs *instance, DenseGraph *molecule_1, DenseGraph *molecule_2, double *predict, int position) {
	...
	static const int nContractions = RisiContraction_18_dropout::nContractions;
